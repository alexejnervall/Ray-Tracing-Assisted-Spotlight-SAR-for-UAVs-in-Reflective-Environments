...
% Position disturbance parameters
sigma_x = 0.00;   % meters (cross-range disturbance)
sigma_z = 0.005;   % meters (altitude disturbance)
...
rangeHistory = zeros(1,numpulses);
trajError = zeros(3,numpulses);
...
for ii = 1:numpulses
    ...
    % Generate position disturbances
    dxTotal = dxTotal + sigma_x* randn;
    dzTotal = dzTotal + sigma_z* randn;

    trajError(:,ii) = [dxTotal;0;dzTotal];
    
    % Apply disturbance (NOT in flight direction)
    radarpos(1) = radarpos(1) + dxTotal;   % cross-range disturbance
    radarpos(3) = radarpos(3) + dzTotal;   % altitude disturbance
...
% %% bp image
% NxBP = 120; NyBP = 120;
% xBP = linspace(-40,40,NxBP);
% yBP = linspace(900,1100,NyBP);
% imgBP = zeros(NyBP,NxBP);
% 
% for ix = 1:NxBP
%     for iy = 1:NyBP
%         pixel = [xBP(ix) yBP(iy) 0];
%         for n = 1:numpulses
%             R = norm(platformPos(n,:) - pixel);
%             tau = 2*R/c;
%             bin = round(tau*fs);
%             if bin>0 && bin<=Nr
%                 imgBP(iy,ix) = imgBP(iy,ix) + cdata(bin,n)*exp(1j*4*pi*R/lambda);
%             end
%         end
%     end
% end
% figure; imagesc(xBP,yBP,abs(imgBP)); axis xy; colormap hot; title("Backprojection SAR Image");